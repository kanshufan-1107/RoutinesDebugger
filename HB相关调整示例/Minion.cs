...
namespace HREngine.Bots
{
    ...
    public class Minion
    {
        ...
        public int locationCooldown = 0;//地标冷却回合

        public bool titanAbilityAvailable = false;
        ...
        public Minion(Minion m)
        {
            ...
            this.locationCooldown = m.locationCooldown;
            this.titanAbilityAvailable = m.titanAbilityAvailable;
            ...
            this.isStarshipLaunchAbility = m.isStarshipLaunchAbility;
        }
        ...
        public void updateReadyness()
        {
            ...
            if (!frozen && (((charge > 0 && rush >= 1 && playedThisTurn)) || !playedThisTurn || shadowmadnessed) && (numAttacksThisTurn == 0 || (numAttacksThisTurn == 1 && windfury) || (!silenced && this.megaWindfury && numAttacksThisTurn <= 3)))
            {
                Ready = true;
                cantAttackHeroes = false;
            }
            if (this.handcard.card.type == CardDB.cardtype.LOCATION && this.locationCooldown == 0)
            {
                Ready = true;
            }
            if (this.handcard.card.type == CardDB.cardtype.LOCATION && this.locationCooldown > 0)
