            ...
            for (int i = startIndex; i < endIndex; i++)  // 不同的index对应同一步的不同选择到达的不同牌面
            {
                ...
                if (!enoughCalculations)
                {
                    ...
                    foreach (Action a in actions)  // 到这步 a.penalty已经计算好了
                    {
                        ...
                        // 新增：检查是否超过了己方随从的面板位置上限
                        if (p.ownMinions.Count >= p.ownMinionsMaxSlots && a.actionType == actionEnum.playcard)
                        {
                            if (a.card.card.type == CardDB.cardtype.MOB || a.card.card.type == CardDB.cardtype.LOCATION)
                            {
                                continue; // 超过己方面板位置上限，跳过该动作
                            }
                        }
